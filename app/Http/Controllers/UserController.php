...
class UserController extends Controller
{
    ...
    public function edit(Request $request){
        $userDetails = Users::select('id','name','email')->where('id',$request->id)->first();
        return view('edit',['userDetails'=>$userDetails]);
    }

    public function update(UpdateUserRequest $request){
        $isUpdated = Users::where('id',$request->id)->update(['name'=>$request->name,'email'=>$request->email]);
        if($isUpdated){
    ...
    public function delete(Request $request){
        $isDeleted = Users::where('id',$request->id)->delete();
        if($isDeleted){
